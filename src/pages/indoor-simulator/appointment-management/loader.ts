...
export const genAppointmentsQuery = (
  ...
  return {
    queryKey: ["appointments", "simulator", storeId, siteId, page],
    queryFn: async () => {
      const response = await privateFetch(
        `/appointment/simulator?populate=storeSimulator&populate=appUser&populate=storeSimulator.store&populate=order&populate=usedCoupon&filter[storeSimulator.store.id]=${storeId}${siteId === "all" ? "" : `&filter[storeSimulator.id]=${siteId}`}&pageSize=10&page=${page}`,
      );
